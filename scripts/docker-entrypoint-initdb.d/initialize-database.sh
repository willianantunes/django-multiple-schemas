...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
  ...
  ALTER ROLE "$APP_ROLE_DEV_DJANGO_MULTIPLE_SCHEMAS" IN DATABASE "$DATABASE_DEV" SET search_path TO $APP_SCHEMA_DEV_DJANGO_MULTIPLE_SCHEMAS;
  ...
  ALTER ROLE "$APP_ROLE_DEV_JAFAR" IN DATABASE "$DATABASE_DEV" SET search_path TO "$APP_SCHEMA_DEV_JAFAR";
  ...
  ALTER ROLE "$APP_ROLE_DEV_IAGO" IN DATABASE "$DATABASE_DEV" SET search_path TO $APP_SCHEMA_DEV_IAGO;
  ...
  ALTER ROLE "$APP_ROLE_DEV_JASMINE" IN DATABASE "$DATABASE_DEV" SET search_path TO $APP_SCHEMA_DEV_JASMINE;
EOSQL
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
  ...
  ALTER ROLE "$APP_ROLE_PRD_DJANGO_MULTIPLE_SCHEMAS" IN DATABASE "$DATABASE_PRD" SET search_path TO $APP_SCHEMA_PRD_DJANGO_MULTIPLE_SCHEMAS;
EOSQL
